...
int CDFileClass::Open(int rights)
{
    ...
    */
    if ((rights & WRITE) && !PathsClass::Is_Absolute(File_Name())) {
        path = PathsClass::Instance().User_Path();
        path += PathsClass::SEP;
        path += File_Name();
...
void CDFileClass::Refresh_Search_Drives(void)
{
    ...
    Add_Search_Drive(PathsClass::Instance().User_Path());
    Set_Search_Drives(RawPath);
    Add_Search_Drive(PathsClass::Instance().Data_Path());
}
        ...
        if (strlen(ptr)) {
            ...
            */
            if (PathsClass::Is_Absolute(ptr)) {
                ...
                std::string fullpath = PathsClass::Instance().User_Path();
                fullpath += ptr;
                ...
                */
                if (PathsClass::Instance().User_Path() != PathsClass::Instance().Data_Path()) {
                    fullpath = PathsClass::Instance().Data_Path();
                    fullpath += ptr;
                    Add_Search_Drive(fullpath.c_str());
                }
...
int CDFileClass::Open(char const* filename, int rights)
{
    ...
    */
    ...
    if ((rights | WRITE)) {
        std::string write_path = PathsClass::Instance().User_Path() + PathsClass::SEP;
        write_path += filename;
        BufferIOFileClass::Set_Name(filename);
        return (BufferIOFileClass::Open(rights));
    }
