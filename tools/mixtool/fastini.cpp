...
const char *FastINIClass::Get_Entry(const char *section, int index) const
{
    ...
    if (sectionptr != nullptr) {
        ...
        if (index < sectionptr->Get_Entry_Count()) {
            for (FastINIEntry *entryptr = sectionptr->EntryList.First(); entryptr != nullptr; entryptr = entryptr->Next()) {
                if (!entryptr->Is_Valid()) {
                    break;
                }
...
int FastINIClass::Enumerate_Entries(const char *section, const char *entry_prefix, uint32_t start_number, uint32_t end_number)
{
    char buffer[256];
...
int FastINIClass::Get_String(const char *section, const char *entry, const char *defvalue, char *buffer, int length) const
{
    FastINIEntry *entryptr;
